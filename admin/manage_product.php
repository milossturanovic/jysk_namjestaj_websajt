...
//ako unesemo sve inpute koje tabela trazi, vrsi se provjera
if(isset($_POST['submit'])){
	...
	$check=mysqli_num_rows($res);
	if($check>0){
		//ako postoji proizvod sa istom id-jem
		if(isset($_GET['id']) && $_GET['id']!=''){
			$getData=mysqli_fetch_assoc($res);
			if($id==$getData['id']){
			
			}else{
				$msg="Proizvod vec postoji";
			}
		}else{
			$msg="Proizvod vec postoji";
		}
	}
	//ako smo nismo unijeli sliku ili pogrijesili format slike
	if($_GET['id']==0){
		if($_FILES['image']['type']!='image/png' && $_FILES['image']['type']!='image/jpg' && $_FILES['image']['type']!='image/jpeg'){
			$msg="Uneiste samo png,jpg,jpeg formate";
		}
	}else{
	...
	//metode sa azuriranje svih polja nekog proizvoda
	if($msg==''){
		if(isset($_GET['id']) && $_GET['id']!=''){
			if($_FILES['image']['name']!=''){
				//slika se skladisti sa nasumicnim brojevima u bazi
				$image=rand(111111111,999999999).'_'.$_FILES['image']['name'];
				move_uploaded_file($_FILES['image']['tmp_name'],PRODUCT_IMAGE_SERVER_PATH.$image);
				//azuriramo svaku kolonu tabele
...
<div class="content pb-0">
            <div class="animated fadeIn">
               <div class="row">
                  <div class="col-lg-12">
                     <div class="card">
                        <div class="card-header"><strong>Proizvod</strong></div>
                        <form method="post" enctype="multipart/form-data">
							<div class="card-body card-block">
							   <div class="form-group">
									<label for="categories" class=" form-control-label">Kategorija</label>
								...
								<div class="form-group">
									<label for="categories" class=" form-control-label">Ime proizvoda</label>
									<input type="text" name="name" placeholder="Enter product name" class="form-control" required value="<?php echo $name?>">
								</div>
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Maloprodajna cijena</label>
									<input type="text" name="mrp" placeholder="Enter product mrp" class="form-control" required value="<?php echo $mrp?>">
								</div>
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Cijena</label>
									<input type="text" name="price" placeholder="Enter product price" class="form-control" required value="<?php echo $price?>">
								</div>
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Kolicina</label>
									<input type="text" name="qty" placeholder="Enter qty" class="form-control" required value="<?php echo $qty?>">
								</div>
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Slika</label>
									<input type="file" name="image" class="form-control" <?php echo  $image_required?>>
								</div>
								
								<div class="form-group">
									<label for="categories" class=" form-control-label">Kratki opis</label>
								...
								<div class="form-group">
									<label for="categories" class=" form-control-label">Opis</label>
